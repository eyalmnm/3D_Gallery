...
def generate_texture_created_success_response(texture):
    return jsonify(
        {'result_code': ErrorCodes.ERROR_CODE_SUCCESS.value, 'error_message': '', 'texture_id': texture.id})
...
def generate_texture_not_found_response(texture_id):
    return jsonify(
        create_error_response(ErrorCodes.ERROR_CODE_TEXTURE_NOT_FOUND, 'texture not found: ' + texture_id))


def generete_default_value_not_found_response():
    return jsonify(
...
def update_texture_by_id(data):
    ...
    if user_id:
        ...
        if texture:
            ...
            texture = texture.update_texture()
...
def get_default_room_texture(data):
    ...
    user_id = get_user_id(uuid=uuid)
    if user_id:
        texture_id = db.session.query_property(Defaults).filter_by(item_type=DefaultItemsIds.DEFAULT_ROOM_TEXTURE_ID)
        if texture_id:
            texture = db.session.query(Texture).get(texture_id).first()
            if texture:
                texture_dict = texture.to_dict()
                return jsonify(
            ...
            return generete_default_value_not_found_response()
    else:
        return generate_user_not_login_response()
...
def update_default_room_texture(data):
    ...
    user_id = get_user_id(uuid=uuid)
    if user_id:
        user = db.session.query(User).filter_by(username=user_id)
        if user and user.status == UserStatus.ADMIN_USER:
            texture_id = db.session.query_property(Defaults).filter_by(
                item_type=DefaultItemsIds.DEFAULT_ROOM_TEXTURE_ID)
            if texture_id:
                texture = db.session.query(Texture).get(texture_id)
                if texture:
                    texture.src = src
                    ...
                    texture = create_texture(DefaultItemsIds.DEFAULT_ROOM_TEXTURE_ID, src, width, height)
                    return generate_texture_created_success_response(texture)
            else:
                texture = create_texture(DefaultItemsIds.DEFAULT_ROOM_TEXTURE_ID, src, width, height)
                return generate_texture_created_success_response(texture)
...
def create_texture(self, item_type, src, width, height):
    texture = Texture(src=src, width=width, height=height)
    texture = texture.save()
    default = db.session.query_property(Defaults).filter_by(item_type=item_type)
    if default:
        default.item_id = texture.id
        default.update_default()
    else:
        default = Defaults(item_type, texture.id)
        default.save()
    return texture
...
def get_default_ceiling_texture(data):
    ...
    user_id = get_user_id(uuid=uuid)
    if user_id:
        texture_id = db.session.query_property(Defaults).filter_by(item_type=DefaultItemsIds.DEFAULT_CEILING_TEXTURE_ID)
        if texture_id:
            texture = db.session.query(Texture).get(texture_id).first()
            if texture:
                texture_dict = texture.to_dict()
...
def update_default_ceiling_texture(data):
    ...
    if user_id:
        ...
        if user and user.status == UserStatus.ADMIN_USER:
            texture_id = db.session.query_property(Defaults).filter_by(
                item_type=DefaultItemsIds.DEFAULT_CEILING_TEXTURE_ID)
            if texture_id:
                texture = db.session.query(Texture).get(texture_id)
                if texture:
                    texture.src = src
                    ...
                    texture = create_texture(DefaultItemsIds.DEFAULT_CEILING_TEXTURE_ID, src, width, height)
                    return generate_texture_created_success_response(texture)
            else:
                texture = create_texture(DefaultItemsIds.DEFAULT_CEILING_TEXTURE_ID, src, width, height)
                return generate_texture_created_success_response(texture)
...
def get_default_wall_texture(data):
    ...
    user_id = get_user_id(uuid=uuid)
    if user_id:
        texture_id = db.session.query_property(Defaults).filter_by(item_type=DefaultItemsIds.DEFAULT_WALL_TEXTURE_ID)
        if texture_id:
            texture = db.session.query(Texture).get(texture_id).first()
            if texture:
                texture_dict = texture.to_dict()
...
def update_default_wall_texture(data):
    ...
    if user_id:
        ...
        if user and user.status == UserStatus.ADMIN_USER:
            texture_id = db.session.query_property(Defaults).filter_by(
                item_type=DefaultItemsIds.DEFAULT_WALL_TEXTURE_ID)
            if texture_id:
                texture = db.session.query(Texture).get(texture_id)
                if texture:
                    texture.src = src
                    ...
                    texture = create_texture(DefaultItemsIds.DEFAULT_WALL_TEXTURE_ID, src, width, height)
                    return generate_texture_created_success_response(texture)
            else:
                texture = create_texture(DefaultItemsIds.DEFAULT_WALL_TEXTURE_ID, src, width, height)
                return generate_texture_created_success_response(texture)
